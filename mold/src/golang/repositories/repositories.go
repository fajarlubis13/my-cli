...
import (
	...
	"{{ toDelimeted .ProjectName 45 }}/helpers/database"
...
func (v *repositories) CreateOne(m *model.{{ toCamel .ProjectName }}) (int64, error) {
	query := "INSERT INTO emonica_v2.{{ toDelimeted .ProjectName 95 }}(kode_jadwal, deskripsi, id_skema_laporan_teknik) VALUES(?, ?, ?) RETURNING id"
...
func (v *repositories) UpdateOneByID(id int64, m *model.{{ toCamel .ProjectName }}) (int64, error) {
	query := "UPDATE emonica_v2.{{ toDelimeted .ProjectName 95 }} SET kode_jadwal = ?, deskripsi = ?, id_skema_laporan_teknik = ? WHERE id = ?"
...
func (v *repositories) GetOneByID(id int64) (*model.{{ toCamel .ProjectName }}, int64, error) {
	query := "SELECT id, kode_jadwal, deskripsi, id_skema_laporan_teknik, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by FROM emonica_v2.{{ toDelimeted .ProjectName 95 }} WHERE id = ?"
...
func (v *repositories) DeleteOneByID(id int64) (int64, error) {
	query := "DELETE FROM emonica_v2.{{ toDelimeted .ProjectName 95 }} WHERE id = ?"
...
func (v *repositories) GetAll(filter *request.QueryParameter) ([]*model.{{ toCamel .ProjectName }}, int64, error) {
	var (
	...
	var (
		...
		mainQuery  = fmt.Sprintf("SELECT %s FROM emonica_v2.{{ toDelimeted .ProjectName 95 }} ", columns)
		countQuery = fmt.Sprintf("SELECT %s FROM emonica_v2.{{ toDelimeted .ProjectName 95 }} ", "COUNT(*) as count")
	...
	var (
		result = make([]*model.{{ toCamel .ProjectName }}, 0)
	)
